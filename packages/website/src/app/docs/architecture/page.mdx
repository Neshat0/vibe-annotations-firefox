...
Browser Extension  ← HTTP sync/CRUD →  MCP Server  ← MCP tools →  AI Agents
   (Firefox)                            (port 3846)               (Claude Code,
                                                                   Cursor, etc.)
```

## Browser Extension

A Firefox WebExtension (MV3) built with vanilla JavaScript. No build step — load the directory as a temporary add-on in Firefox Developer Edition or Nightly.
...
1. User creates an annotation in the extension (stored in Firefox storage)
